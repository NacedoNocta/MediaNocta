...
using Backoffice.Authentication.Configuration;
using Backoffice.Authentication.Interfaces;
using Backoffice.Authentication.Services;
using CustomAuthenticationOptions = Backoffice.Authentication.Configuration.AuthenticationOptions;
using ICustomAuthenticationService = Backoffice.Authentication.Interfaces.IAuthenticationService;

namespace Backoffice.Authentication.Extensions;
...
public static class AuthenticationServiceExtensions
{
    public static IServiceCollection AddCustomAuthentication(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        .AddCookie("Cookies", options =>
        {
            ...
            options.AccessDeniedPath = "/403";  // Redirect to 403 Forbidden page
        })
            ...
            options.Events = new Microsoft.AspNetCore.Authentication.OpenIdConnect.OpenIdConnectEvents
            {
                ...
                OnTokenValidated = async context =>
                {
                    ...
                    try
                    {
                        ...
                        // Get account with roles to add them to claims
                        var accountWithRoles = await accountService.GetAccountByIdAsync(localAccount.Id);
                        ...
                        // Store session ID and account info in claims for later retrieval
                        ...
                        // Add role claims from database to the identity (for authorization middleware)
                        if (accountWithRoles != null)
                        {
                            foreach (var accountRole in accountWithRoles.LocalAccountRoles)
                            {
                                identity?.AddClaim(new System.Security.Claims.Claim(System.Security.Claims.ClaimTypes.Role, accountRole.Role.Name));
                            }

                            logger.LogInformation("Added {RoleCount} role claims to user {Username}",
                                accountWithRoles.LocalAccountRoles.Count, localAccount.Username);
                        }
        ...
        // Add authorization with fallback policy requiring authentication and backoffice-admin role
        services.AddAuthorization(options =>
        {
            // Require authentication and backoffice-admin role for all endpoints by default
            options.FallbackPolicy = new Microsoft.AspNetCore.Authorization.AuthorizationPolicyBuilder()
                .RequireAuthenticatedUser()
                .RequireRole("backoffice-admin")
                .Build();

            // Policy for logout - only requires authentication, not the role
        ...
        services.AddScoped<ICustomAuthenticationService, Backoffice.Authentication.Services.AuthenticationService>();
